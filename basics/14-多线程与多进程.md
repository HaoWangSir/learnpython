...
在Python中我们可以使用`threading`模块编写多线程程序. 由于`GIL(Global Interpreter Lock)`的影响使得Python多线程不适宜用来处理**CPU密集型**的任务，但对于**IO密集型**任务，Python多线程则可以胜任. **CPU密集型**任务一般指的是高度使用CPU进行计算的任务，如科学计算，**IO密集型**任务一般指的是要频繁处理IO的任务，如网络服务. 与多线程不同的是，多进程不受`GIL`的影响，不管是CPU密集型还是IO密集型都可以使用多进程提升程序执行效率. 下面我们通过一个例子演示如何在Python中使用多线程和多进程. 下面这个多线程和多进程程序中我们启动多个线程和进程对清华大学开源的新闻数据集进行分词，因此在运行这个例子之前我们需要下载该数据集，[下载地址](http://thuctc.thunlp.org/)，这里分词我们使用的是[jieba](https://github.com/fxsjy/jieba)，安装`jieba`的方式很简单，通过`pip install jieba`即可安装`jieba`
